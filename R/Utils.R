...
#' 
#' @importFrom flowWorkspace gh_pop_get_data cytoframe_to_flowFrame
#' 
#' @return List.
#' @export
#' @examples A <- 2+2
...
magic_label_poly<-function(df,polygon_df,label_pol="1"){
  ...
  inside <- point.in.polygon(df[,1], df[,2], polygon_df[,1], polygon_df[,2])
...
flowmagic_pred_to_poly_gates<-function(list_df,pred_label,gate_label,n_cores=1,concavity_val=10){
  ...
  list_poly_gates<-parallel::mclapply(seq_along(list_df),function(i){
    ...
    pg <- polygonGate(filterId = gate_label,
                      .gate = coords,
...
flowmagic_pred_to_gs<-function(list_poly_gates,gs,parent_node){
  ...
  for(i in seq_along(list_poly_gates)){
    ...
    print(current_poly)
    suppressWarnings({
      gs_pop_add(gs[[current_sample]], current_poly, parent = parent_node, name = current_poly@filterId)
      # may just be add?
    })
