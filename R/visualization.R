...
magicPlot<-function(df, type = "dens", polygons_coords_list = NULL, show_legend = TRUE, 
  ...
  if (type == "no_gate" || ncol(df) == 2) {
    ...
    if(is.null(x_lab)==TRUE && is.null(y_lab)==TRUE){
      p <- p + xlab(original_col_names[1]) + ylab(original_col_names[2])
    }else if(is.null(x_lab)==FALSE || is.null(y_lab)==FALSE){
...
magicPlot_template<-function(df,size_points=1){
  ...
  pts <- locator(type = "o")  # type = "n" means collect points only, not draw
