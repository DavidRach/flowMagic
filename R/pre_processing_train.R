...
#' 
#' @importFrom flowCore sampleNames
#' @importFrom flowWorkspace gh_pop_get_data gh_pop_get_indices gh_pop_get_gate gs_pop_get_parent
...
map_to_root<-function(gh,pop){
  ...
  }else if(inherits(gh, "GatingHierarchy")){
    gh<-gh
  }
  print("Input correct")
  ...
  inds_selected_pop<-which(logic==TRUE)
  ...
  if(pop!="root"){ # root is not a gate, so the dimensions are not important
    ...
    if(inherits(polgate, "polygonGate")){
      dims<-colnames(polgate@boundaries)
    }else if(inherits(polgate, "rectangleGate")){
      rectGate<-polgate
      vec<-rectGate@min
      dims<-names(vec)
    ...
    }else if(inherits(polgate, "booleanFilter")){
      ...
      if(inherits(polgate_parent, "polygonGate")){
        dims<-colnames(polgate_parent@boundaries)
      }else if(inherits(polgate_parent, "rectangleGate")){
  ...
  if(length(child_pops_x_list)>=1){
    ...
    s<-sapply(1:length(child_pop_X_vector),function(i){
      ...
      if(inherits(polgate, "polygonGate")){
        ...
        }else if(inherits(polgate_parent, "booleanFilter")){
...
#' @param seed_n Numeric for set.seed, default is 123
#' 
#' @importFrom flowWorkspace gs_pop_get_parent
#' @importFrom magrittr %>%
#' @importFrom stringr str_replace
#' @importFrom data.tree ToDataFrameTree as.Node
#' @importFrom randomcoloR distinctColorPalette
#' @importFrom visNetwork visNetwork visEdges visHierarchicalLayout visNodes visOptions
#' 
...
get_hierarchy_all_pops<-function(gh,export_visnet=FALSE,path.output="None", seed_n=123){
  start<-Sys.time()
  if (("package:stringr" %in% search()) == FALSE) {
    #warning("stringr library not loaded, attempt to loading...")
    #library(stringr)
  }
  if (("package:data.tree" %in% search()) == FALSE) {
    #warning("data.tree library not loaded, attempt to loading...")
    #library(data.tree)
  }
  if (("package:visNetwork" %in% search()) == FALSE) {
    #warning("visNetwork library not loaded, attempt to loading...")
    #library(visNetwork)
  }
    if (("package:randomcoloR" %in% search()) == FALSE) {
  ...
  #-------------------- export visnetwork -------------------------
  if(export_visnet==TRUE){
    dir.create(paste0(path.output,"/Gatinghierarchy_Plots/"),recursive = TRUE)
  }
  end<-Sys.time()
  ...
  for(level in 2:max_level){ # we start from the second level, because the root has no mother.
    ...
    if(length(inds_same_dims_pops)>0){
      ...
      for(element in set_same_dims){
        ...
        inds<-which((df_tree$Dimensions %in% element)==TRUE)
